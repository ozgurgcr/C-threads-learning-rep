...
void* read_thread(){             /*burda okuma ve yazma yapabilmek için bir rw_lock kullanmam gerekecek sanırım veya bir mutex kullanmalıyım*/
    ...
    while(1){
        ...
        if (num_bytes > 0){
            //rx_buffer[num_bytes] = 0x0;/*********************************************************/
            //rx_buffer[num_bytes - 1] = 0x0;/*****************************************************/
            printf("%s", rx_buffer);
            fflush(stdout);             /*it prints right when code executed doesnt wait for \n or something.*/
        }
        // else if (num_bytes < 0){
...
int main(){
    ...
    serial_port_config.c_iflag &= ~(IGNBRK | BRKINT | INLCR | IGNCR | ICRNL | IXON);
    serial_port_config.c_iflag |= IGNCR | IUTF8;
    serial_port_config.c_oflag &= ~(OPOST | OLCUC | ONLCR | OCRNL | ONOCR | ONLRET | OFILL | OFDEL );
    serial_port_config.c_oflag |= ONLRET | ONOCR; 
    ...
    serial_port_config.c_lflag &= ~CREAD;
    serial_port_config.c_cc[VMIN] = 1;
